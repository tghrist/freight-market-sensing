...
* **`dow_jones_transportation_avg`** (Engineered Features: `djta_weekly_mean`, `djta_weekly_last`, `djta_weekly_volatility`)
    * **Definition:** Dow Jones Transportation Average. A high-frequency, forward-looking stock market index tracking the performance of 20 large U.S. transportation companies. The daily data is aggregated into weekly mean (trend), last close (verdict), and volatility (panic score) to prevent noise.
    * **S&OP Context:** An early-warning sentiment indicator. Because stock prices reflect expected future earnings, major freight carriers typically see their stock prices drop 6 to 9 months before they physically cancel OEM trailer orders.
