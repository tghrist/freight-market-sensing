...
class EquipmentDemandForecaster:
    def __init__(self, forecast_horizon_weeks):
    ...
    def build_model(self):
        ...
        print("-" * 50)
        print(f"Building {self.forecast_horizon_weeks}-Week Advance Forecasting Model...")
        ...
        ml_df = self.df.copy()
        ml_df['future_target'] = ml_df[self.target_col].shift(-self.forecast_horizon_weeks)
        ...
        print(f"Validation MAPE (Predicting {self.forecast_horizon_weeks} weeks out): {mape:.2%}")
    ...
    def generate_forecast(self):
        ...
        print(f"Predicted Trailer Production Index in {self.forecast_horizon_weeks} weeks: {prediction:.2f}")
        print("-" * 50)


if __name__ == "__main__":
    nowcaster = EquipmentDemandForecaster(forecast_horizon_weeks=0)
    X_train, y_train, X_test, y_test, test_preds = nowcaster.build_model()
    nowcaster.generate_forecast()